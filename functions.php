...
function bryhub_simple_setup() {
	/*
		...
		* If you're building a theme based on bryhub-simple, use a find and replace
		* to change 'bryhub-simple' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'bryhub-simple', get_template_directory() . '/languages' );
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'bryhub_simple_custom_background_args',
			array(
...
function bryhub_simple_scripts() {
	wp_enqueue_style( 'bryhub-simple-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'bryhub-simple-style', 'rtl', 'replace' );

	wp_enqueue_script( 'bryhub-simple-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
